...
if __name__ == "__main__":
    ...
    data = pd.read_csv('ml-latest-small/ratings.csv', nrows=10000, header=None, engine='python')
    # 导入数据
    ...
    train, test = SplitData(d, 8, 2, 8)
    t = []
    for key, item in train.items():
        t.append(pd.Series(name=key, data=item))
    train = pd.DataFrame(t)
    t = []
    t2 = []
    for key, item in test.items():
        t2.append(pd.Series(name=key, data=item))
    test = pd.DataFrame(t2)
    t2 = []
